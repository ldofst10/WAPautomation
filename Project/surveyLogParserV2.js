...
rl.on('line', (line) => {
    ...
    for(const idx in data) {
        if(data[idx].indexOf('—') !== -1) { // 이름 파싱하고 각 객체 생성
            toWrite.push({'name':data[idx].split('—')[0]});
            ...
            for(const inf in tmpObj) {
                if(toWrite[toWrite.length-1].info[inf] !== undefined){
                    toWrite[toWrite.length-1].info[inf] += (', '+ tmpObj[inf]);
                } else {
                    toWrite[toWrite.length-1].info[inf] = tmpObj[inf];
                }
            }
    ...
    for(const idx in toWrite) {
        str2 += toWrite[idx].name + '\t';
            for(const tle in columnList) {
                if(toWrite[idx].info[columnList[tle]]!=undefined)
                    str2 += toWrite[idx].info[columnList[tle]];
                str2 += '\t';
            }
        str2 += '\n';
    }
